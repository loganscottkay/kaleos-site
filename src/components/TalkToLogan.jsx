...
export default function TalkToLogan() {
  ...
  const send = async () => {
    ...
    turns.current += 1;
    try {
      const res = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: updated.map((m) => ({ role: m.role, content: m.content })), session_id: sessionId.current }) });
  ...
  return (<>
    ...
    {isOpen&&(<div style={{position:"fixed",bottom:24,right:24,width:"min(440px,calc(100vw - 48px))",height:"min(680px,calc(100vh - 48px))",zIndex:10001,display:"flex",flexDirection:"column",background:"linear-gradient(180deg,#0f1a2e,#162033)",borderRadius:20,border:"1px solid rgba(255,255,255,.08)",boxShadow:"0 24px 80px rgba(0,0,0,.5),0 0 0 1px rgba(13,148,136,.1)",overflow:"hidden",animation:"ltlFadeUp .3s cubic-bezier(.4,0,.2,1)"}}>
      <div style={{padding:"20px 20px 16px",display:"flex",alignItems:"center",gap:14,background:"linear-gradient(135deg,rgba(13,148,136,.06),rgba(27,42,74,.4))",borderBottom:"1px solid rgba(255,255,255,.06)"}}><div style={{width:44,height:44,flexShrink:0,position:"relative"}}><img src="/photo.png" alt="Logan Kay" style={{width:44,height:44,borderRadius:"50%",objectFit:"cover"}}/><div style={{position:"absolute",bottom:1,right:1,width:10,height:10,borderRadius:"50%",background:"#22c55e",border:"2px solid #0f1a2e"}}/></div><div style={{flex:1}}><div style={{color:"white",fontFamily:"var(--font-fraunces),Georgia,serif",fontSize:17,fontWeight:600}}>Logan Kay</div><div style={{color:"rgba(255,255,255,.4)",fontFamily:"'Inter',sans-serif",fontSize:12,marginTop:2}}>Founder, Kaleos</div></div><button onClick={()=>setIsOpen(false)} style={{width:32,height:32,borderRadius:"50%",border:"none",background:"rgba(255,255,255,.06)",color:"rgba(255,255,255,.5)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontFamily:"sans-serif"}}>✕</button></div>
      ...
      {showCTA&&(<div style={{padding:"10px 20px 4px"}}><div style={{padding:"10px 14px",borderRadius:"14px",background:"rgba(13,148,136,.08)",border:"1px solid rgba(13,148,136,.15)",color:"rgba(255,255,255,.75)",fontFamily:"'Inter',sans-serif",fontSize:13,lineHeight:1.5,marginBottom:10}}>Sounds like we should talk! Here&apos;s how to take the next step:</div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button onClick={()=>window.open("https://calendly.com/logan-kaleoshq/30min","_blank")} style={{padding:"9px 16px",borderRadius:20,border:"none",background:"linear-gradient(135deg,#0d9488,#0f766e)",color:"white",fontFamily:"'Inter',sans-serif",fontSize:13,fontWeight:600,cursor:"pointer",transition:"all .2s"}}>Book a free call</button><button onClick={()=>window.open("mailto:logan@kaleoshq.com?subject=Interested%20in%20Kaleos&body=Hi%20Logan%2C%20I'd%20like%20to%20learn%20more%20about%20how%20Kaleos%20can%20help%20my%20business.")} style={{padding:"9px 16px",borderRadius:20,border:"1px solid rgba(13,148,136,.3)",background:"rgba(13,148,136,.1)",color:"#5eead4",fontFamily:"'Inter',sans-serif",fontSize:13,fontWeight:500,cursor:"pointer",transition:"all .2s"}}>Email me directly</button></div></div>)}
      <div style={{padding:"14px 20px 20px",borderTop:"1px solid rgba(255,255,255,.06)",display:"flex",gap:10,alignItems:"flex-end"}}><textarea ref={inputRef} value={input} onChange={(e)=>setInput(e.target.value)} onKeyDown={(e)=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}}} placeholder="Tell me about your business..." rows={1} style={{flex:1,padding:"12px 16px",borderRadius:14,border:"1px solid rgba(255,255,255,.1)",background:"rgba(255,255,255,.04)",color:"white",fontFamily:"'Inter',sans-serif",fontSize:14,outline:"none",resize:"none",lineHeight:1.4}}/><button onClick={send} disabled={!input.trim()||loading} style={{width:42,height:42,borderRadius:12,border:"none",background:input.trim()?"linear-gradient(135deg,#0d9488,#0f766e)":"rgba(255,255,255,.06)",color:input.trim()?"white":"rgba(255,255,255,.2)",cursor:input.trim()?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all .2s"}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg></button></div>
