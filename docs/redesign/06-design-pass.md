...
## v4 changes (2026-09-17, tag cp7-v4)

Sections flow without eyebrows or hard rules (a soft horizon line instead). Header is a rail: bigger KALEOS, mono links with a live dot, a starred ghost call, hides on the way down. Sourced statistics section replaces "the implementation gap" prose, every figure linked. Orbit rebuilt with the business at the center, four stations, the gate, sticky beside the scrolling phases. Founder line links to About. LinkedIn in the footer, a privacy page, Organization, Service, and FAQPage structured data. Favicon regenerated at 256 with the K at 74 percent. OG image and a LinkedIn and X kit regenerated from the supplied K. Focus group and critic findings applied: thirty days and pricing on the first screen, the gate explained beside the diagram, a plainer outcome label on the form, a write-first path from the close, one fewer sales beat in the stats. Checks after: 0 contrast failures on six routes, no overflow at 320, 390, 768, 1440, 27 keyboard stops on home with visible focus, all internal links and anchors resolve, structured data parses.
